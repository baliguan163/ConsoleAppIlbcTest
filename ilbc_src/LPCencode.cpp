...
*  lsf quantizer (subrutine to LPCencode)
   float *lsfdeq,    (o) dequantized lsf coefficients (dimension FILTERORDER) 
...
void SimplelsfQ(float *lsfdeq, int *index, float *lsf, int lpc_n)
{
    ...
    if (lpc_n == 2)
    {
       ...
       SplitVQ(lsfdeq + LPC_FILTERORDER, index + LSF_NSPLIT,
               lsf + LPC_FILTERORDER, lsfCbTbl, LSF_NSPLIT, dim_lsfCbTbl, size_lsfCbTbl);
    }
}
...
void LPCencode(float *syntdenum, float *weightdenum, int *lsf_index, float *data, iLBC_Enc_Inst_t *iLBCenc_inst)
{
   ...
   SimpleInterpolateLSF(syntdenum, weightdenum, lsf, lsfdeq, iLBCenc_inst->lsfold,
