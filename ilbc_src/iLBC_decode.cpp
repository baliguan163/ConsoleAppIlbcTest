...
short initDecode(iLBC_Dec_Inst_t *iLBCdec_inst, int mode, int use_enhancer)
{
    ...
    memcpy((*iLBCdec_inst).lsfdeqold, lsfmeanTbl, LPC_FILTERORDER*sizeof(float));
    memset(iLBCdec_inst->old_syntdenum, 0, ((LPC_FILTERORDER + 1)*NSUB_MAX)*sizeof(float));
...
*  frame residual decoder function (subrutine to iLBC_decode)
   ...
   int *idxVec,                     (i) codebook indexes for the samples  in the start state 
   ...
   int *extra_cb_index,             (i) the indexes for the adaptive codebook part of start state 
...
void Decode(iLBC_Dec_Inst_t *iLBCdec_inst, float *decresidual, int start, int idxForMax, 
            int *idxVec, float *syntdenum, int *cb_index, int *gain_index, int *extra_cb_index, 
            int *extra_gain_index, int state_first)
{
    ...
    if (state_first) 
    {
        ...
        memcpy(mem+CB_MEML-iLBCdec_inst->state_short_len, decresidual+start_pos, iLBCdec_inst->state_short_len*sizeof(float));
...
void iLBC_decode(float *decblock, unsigned char *bytes, iLBC_Dec_Inst_t *iLBCdec_inst, int mode)
{
    ...
    if (mode>0)
    {
        ...
        if (mode==1) 
        {
            ...
            DecoderInterpolateLSF(syntdenum, weightdenum, lsfdeq, LPC_FILTERORDER, iLBCdec_inst);

            Decode(iLBCdec_inst, decresidual, start, idxForMax,
                   idxVec, syntdenum, cb_index, gain_index, extra_cb_index, extra_gain_index, state_first);
