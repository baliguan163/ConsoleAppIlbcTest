...
void iCBSearch(iLBC_Enc_Inst_t *iLBCenc_inst, int *index, int *gain_index, float *intarget, 
               float *mem, int lMem, int lTarget, int nStages, float *weightDenum, 
               float *weightState, int block)
{
        ...
        */
        ...
        if (lTarget==SUBL)
        {
            ...
            searchAugmentedCB(20, 39, stage, base_size-lTarget/2,
                              target, buf+LPC_FILTERORDER+lMem, &max_measure, &best_index, &gain, energy, invenergy);
        }
        ...
        if ((lTarget==SUBL)&&(sIndAug!=0)) 
        {
            searchAugmentedCB(sIndAug, eIndAug, stage,2*base_size-20,
                              target, cbvectors+lMem, &max_measure, &best_index, &gain, energy, invenergy);
        }
        ...
        else 
        {
            if (stage==1)
            {
                gain = gainquant(gain, (float)fabs(gains[stage-1]), 16, &gain_index[stage]);
            }
            else 
            {
